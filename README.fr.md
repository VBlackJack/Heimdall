...
*Ce document est la version française de [README.md](README.md). / This document is the French version.*
...
**Le gestionnaire de connexions Windows tout-en-un et sécurisé pour RDP, SSH, SFTP, VNC, Telnet, FTP, Citrix, WinRM et les terminaux locaux.**

Construit avec .NET 10 et WPF. Gestionnaire de connexions Windows sécurisé et complet, avec un chiffrement de niveau entreprise et une ergonomie moderne.

---

## Pourquoi Heimdall ?

- **9 protocoles, une seule interface** --- RDP, SSH, SFTP, VNC, Telnet, FTP, Citrix, WinRM et sessions shell locales dans une unique fenêtre à onglets
- **Stockage des identifiants en zero-trust** --- DPAPI + intégrité HMAC-SHA256, un **coffre à mot de passe maître** optionnel (dérivation de clé Argon2id, AES-256-GCM) avec verrou de déverrouillage au démarrage et verrouillage du plan de travail sur inactivité, protection du code PIN par PBKDF2, application des ACL Windows et, en option, **Windows Hello** (biométrie/PIN) pour déverrouiller le coffre et comme barrière avant connexion
- **Intégration de coffres externes** --- KeePassXC, KeePass2 (KPScript), Bitwarden CLI, 1Password CLI, `pass`, le Gestionnaire d'identifiants Windows natif, ou tout gestionnaire de mots de passe en ligne de commande
- **Pageant natif** --- IPC direct avec PuTTY Pageant via mémoire partagée (sans bricolage de type agent forwarding)
- **Distribution portable** --- build autonome sans installateur requis ; les données modifiables sont stockées par utilisateur sous `%LOCALAPPDATA%\Heimdall`
...
- Limitation connue des passerelles : certains environnements acceptent le tunnel au niveau TCP mais la cible (ou un équipement intermédiaire) ferme l'échange HTTP WinRM - diagnostiqué comme environnemental, non imputable à Heimdall. Voir [docs/fr/winrm-gateway-12152-diagnostic.md](docs/fr/winrm-gateway-12152-diagnostic.md).
...
### Boîte à outils sysops intégrée (59 outils)
...
- **Opérations en masse** : sélection multiple (Ctrl+Clic, Maj+Clic) -> clic droit -> connexion groupée, duplication, suppression, déplacement vers un projet/groupe, modification du port, du nom d'utilisateur, du mot de passe (chiffré DPAPI, avec boîte de confirmation) et **modification groupée de la passerelle SSH** (sans identifiants, quatre résultats explicites - conserver / forcer en direct / hériter / spécifique - en ignorant les protocoles qui ne gèrent pas les passerelles)
...
- **Filtres d'arborescence structurés** : filtrez l'arborescence des sessions par protocole/type, favori et état de connexion, en combinaison avec la recherche textuelle temporisée, le tout en une passe versionnée sur des nœuds stables ; une pastille colorée sur le bouton de filtre signale un filtre actif
...
- Hiérarchie TreeView : Projet > Groupe > Serveur, avec icônes d'outils colorées par catégorie et pastilles d'état
...
- Héritage de connexion : valeurs par défaut au niveau du groupe pour la passerelle, le nom d'utilisateur SSH et le chemin de clé
...
- Bouton d'aide intégré ("?") sur les 59 outils, avec instructions d'utilisation localisées
...
- Les limitations de sécurité connues et les notes de modèle de menace sont suivies dans [docs/fr/SECURITY.md](docs/fr/SECURITY.md)
...
Deux éditions sont disponibles. Toutes deux embarquent le runtime .NET complet et **ne nécessitent aucune installation .NET préalable**.

| Édition | Taille | WebView2 | Recommandée pour |
|---------|------|----------|----------|
| **Standard** | ~106 MB (installateur) / ~159 MB (zip) | Nécessite Edge ou le WebView2 Evergreen Runtime (préinstallé sur Windows 10/11) | La plupart des utilisateurs - postes de travail, portables, tout PC doté d'Edge |
| **Self-Contained** | ~267 MB (installateur) / ~380 MB (zip) | Embarqué (WebView2 Fixed Version Runtime inclus) | Serveurs isolés du réseau, environnements restreints sans Edge, VM cloisonnées |

Les deux éditions sont proposées en **installateur** (.exe avec raccourcis, détection de mise à niveau, désinstallateur) ou en **zip** (extraire et exécuter, sans installation).

> **Laquelle choisir ?** En cas de doute, prenez **Standard**. Elle fonctionne sur n'importe quel PC Windows 10/11 où Edge est installé (c'est-à-dire la quasi-totalité). Ne choisissez **Self-Contained** que si la machine cible n'a ni accès Internet ni navigateur Edge.
...
|---|---|---|
| Windows | 10 / 11 | Les deux éditions |
| Edge ou WebView2 Runtime | Evergreen | Édition Standard uniquement (préinstallé sur Windows 10/11) |
| PuTTY (Plink + Pageant) | 0.81+ | Optionnel, pour l'authentification SSH par clé via Pageant uniquement |
| Serveur X11 | VcXsrv / Xming / X410 | Optionnel, pour la redirection X11 |
| Citrix Workspace App | Dernière version | Optionnel, pour les connexions Citrix |

---

## Démarrage rapide

Téléchargez la dernière version depuis la page [Releases](../../releases). Exécutez l'installateur, ou extrayez le zip et lancez `Heimdall.exe`.
...
| Raccourci | Action |
|----------|--------|
| F1 | Aide sur les raccourcis clavier |
| Ctrl+K | Palette de connexion rapide (serveurs, outils, outils externes) |
| Ctrl+Shift+T | Basculer l'onglet de la barre latérale (Sessions / Outils) |
| Ctrl+N | Ajouter un serveur |
| Ctrl+E | Modifier le serveur sélectionné |
| Ctrl+Del | Supprimer le serveur sélectionné |
| Ctrl+Shift+N | Scanner réseau |
| Ctrl+Shift+S | Capture d'écran vers le presse-papiers |
| Ctrl+Shift+O | Basculer l'orientation du split (H/V) |
| Ctrl+B | Basculer la barre latérale |
| Ctrl+F | Placer le focus dans la recherche/le filtre |
| Ctrl+Enter | Exécuter l'action (JSON Formatter, etc.) |
| F11 | Basculer en plein écran |
| Escape | Quitter le plein écran / fermer la palette |
| F2 | Renommer (navigateur de fichiers SFTP/local) |
| F5 | Rafraîchir le répertoire |

---

## Compilation depuis les sources

**Raccourcis batch** (double-clic) :

| Fichier | Action |
|------|--------|
| `Run.bat` | Compilation + lancement en mode Debug |
| `Test.bat` | Exécuter tous les tests |
| `Build.bat` | Compilation Debug avec publication |
| `Release.bat` | Release complète : build, tests, packaging, commit, push, release GitHub |
...
Les artefacts de compilation sont écrits dans `Dist/debug/` ou `Dist/release/`, dans des dossiers dont le nom porte la version.

| Édition | Taille (zip / installateur) | WebView2 |
|---------|----------------------|----------|
| **Standard** | ~159 MB / ~106 MB | Nécessite Edge/WebView2 du système |
| **Self-Contained** | ~380 MB / ~267 MB | Fixed Version Runtime embarqué |

Le mode Release produit également des installateurs Inno Setup `.exe` dans `Dist/installers/`, avec raccourcis bureau/menu Démarrer et détection de mise à niveau.
...
| Outils intégrés | 59 outils sysops (Ctrl+K -> `tools` ou Ctrl+Shift+T) |
...
Voir [docs/fr/ARCHITECTURE.md](docs/fr/ARCHITECTURE.md) pour les décisions de conception détaillées et les diagrammes de flux de données.
...
Distribué sous licence Apache, version 2.0. Voir [LICENSE](LICENSE) pour les détails.
