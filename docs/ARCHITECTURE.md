...
Heimdall is a .NET 10 WPF application organized as a multi-project solution with strict dependency boundaries. Supports RDP, SSH, SFTP, FTP, VNC, Telnet, Citrix, and Local Shell connection types with 6,148 i18n keys per locale (EN/FR), 58 built-in sysops tools with contextual help, cross-tool navigation, and 10,801 automated tests (10,670 of them gating in CI and all passing, the remaining 131 in the informational CIUnstable and RequiresDesktop lanes). Health monitor polls in parallel (Task.WhenAll), XML importers hardened against XXE, all Debug.WriteLine replaced with FileLogger. WCAG AA compliant Design System with 45 design tokens (typography min 11px, spacing, corner radius, opacity, icon sizes, font family), micro-animations, FocusIndicatorBrush for keyboard accessibility, unified two-tier icon system (vector geometries + MDL2), per-category tool color coding, declarative i18n via `{loc:Translate}` markup extension, and progressive disclosure ServerDialog.
...
Trust store changes reach `settings.json` through `HostKeyPersistenceCoalescer` (`Heimdall.App/Services/`): every `EntryTrusted` / `EntryReplaced` / `EntryRemoved` and `HostKeyStore.HostKeyEvent` is queued, and one `MergeSettingAsync` writes the batch after a 250 ms quiet window, last change per key winning; a flush is started at exit before the first await and awaited at the end. `HostKeyTrustService` only scans the hashed entries for a plain candidate host (a hashed token can match nothing but itself) and enumerates the store without copying it. On the session side, `SshShellSession` reaches the network through an internal `Transport` record (host key resolution, client creation, handshake, shell stream) so tests drive its read loop with a scripted stream; every await of `ConnectAsync` is followed by a disposed check that releases what a concurrent `Dispose` could not see. A pane's tunnel port is handed out once by `ConnectionStateMachine.TryTakeTunnelLocalPort`, so the process-exit thread and the UI close cannot both release the same acquisition, and `TunnelManager.AcquireReusableTunnel` honours the distinct-loopback preference of external launches.
...
**KeePassXC key-file authentication**: enterprise KeePassXC databases authenticate with a key file (`.keyx`/`.key`), used alone or together with a master password. A `{KeyFile}` placeholder and the `CredentialProviderKeyFile` setting (a plaintext file path, not a secret) feed the key file into the command via `-k "{KeyFile}"`. The two path placeholders `{Database}` and `{KeyFile}` use a **path-aware sanitizer** for non-shell targets that strips only the double quote and CR/LF, instead of the broader relaxed set. The rationale: the provider runs with `UseShellExecute=false`, so no shell interprets the arguments, and the double quote (illegal in Windows filenames) is the sole argument-boundary metacharacter; stripping only it preserves every legal path character. This also fixes latent corruption of `{Database}` paths containing characters such as `&` or `$`. Three KeePassXC presets ship: `"KeePassXC"` (master password, now with `-q`), `"KeePassXC (key file)"` (master password + key file), and `"KeePassXC (key file only)"` (`--no-password`, key file alone). In Settings, a **Key file** field with a Browse dialog (`*.keyx;*.key`) sits beside the database field, and the **Test** button warns up front when a `{KeyFile}` template is selected with no key file path, before launching the tool. EN/FR locale keys are added at parity. The presets are validated against the real `keepassxc-cli` 2.7.11.
...
- **Package source**: Heimdall consumes `ThemeForge.Theme` from the public nuget.org feed (anonymous restore; no token). ThemeForge owns the 17 palette dictionaries and injects the active palette into `Application.Resources.MergedDictionaries`.
