...
- The three WebView2 surfaces that serve a local bundle now expose their asset folder only to a
  document of their own origin instead of to any origin. Each was driven under the stricter setting
  first: draw.io loads whole and exports a valid PNG, the Markdown editor renders, which was the one
  in doubt because its bundle loads a module script declared cross-origin, and a VNC session against
  a live target paints its desktop and stays up.
