...
- **Import RDP unifié** : les fichiers `.rdp` déposés sur la fenêtre principale ou importés depuis `Settings -> Import` empruntent le même flux d'aperçu et de résolution de conflits ; remplacer un profil existant conserve tous les champs que le fichier ne peut pas porter (résolution fixe, mot de passe enregistré, étapes post-connexion) ; l'import lit les clés que le client Bureau à distance écrit lui-même quand il enregistre un fichier (`redirectdrives`, `domain`, ports COM, micro, session console, compression, cache bitmap, reconnexion automatique, USB, caméra, résolution dynamique)
...
- **Mode sudo "Parcourir en root"** : une bascule dans la barre d'outils liste les répertoires avec GNU find et des champs séparés par NUL via SSH. Les noms contenant une flèche sont préservés ; les noms dangereux sont exclus
- **Repli sudo complet** sur toutes les opérations : téléversement (préparation privée et publication atomique), téléchargement (`sudo cat`), édition, chmod, renommage, suppression, mkdir - déclenché uniquement sur des exceptions typées de permission refusée
- Les transferts remplacent un fichier uniquement après un choix explicite. Les nouvelles destinations et les noms automatiques refusent les collisions tardives. La création distante sûre exige SFTP avec un canal de commandes SSH fonctionnel ; les téléversements créant un fichier en FTP sont refusés
- Les remplacements SFTP conservent le GID POSIX et les bits de permission ; un GID non reproductible fait refuser le remplacement. Fermer le volet annule aussi les ouvertures d'éditeur externe encore en cours de téléchargement
...
- **Recherche et sélection dans l'arbre** : filtres retirables, réinitialisation et actions sans résultat, dossier et hôte dans les résultats, actions visibles pour la sélection multiple.
- **Organisation guidée** : indication de destination, ouverture au survol et défilement pendant le glisser-déposer ; annulation du dernier déplacement, renommage dans l'arbre ou changement d'ordre, avec contrôle des conflits.
...
- **Couleurs de dossier** : le menu du dossier propose les huit couleurs de la palette des badges de projet, plus un retour à la couleur par défaut ; la couleur peint l'icône du dossier, est héritée par les sous-dossiers tant qu'ils n'en choisissent pas une, suit un renommage et disparaît avec la suppression
- **Déplacement de dossiers par glisser-déposer**, ou depuis le sous-menu "Déplacer vers" du dossier : déposé sur un autre dossier il en devient un sous-dossier, déposé sur la zone de dépôt au-dessus de l'arborescence il revient au premier niveau ; sous-dossiers, sessions, couleur et réglages voyagent avec lui, et un dépôt sur le dossier lui-même, un sous-dossier ou le parent actuel est refusé
- **Ordre manuel des sessions** : une session déposée sur une autre ligne se place avant ou après elle (un trait accentué montre le côté), un dépôt sur une ligne d'un autre dossier déplace et positionne en une seule écriture, Alt+Haut et Alt+Bas déplacent d'un cran la session en surbrillance ; un ordre de tri à zéro signifie "pas d'ordre manuel" et se classe après toutes les sessions ordonnées
...
- Interface trilingue : anglais, français et espagnol (6 431 clés i18n par langue, parité exacte entre les trois catalogues)
