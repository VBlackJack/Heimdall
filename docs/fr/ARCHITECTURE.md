...
Heimdall est une application WPF .NET 10 organisée en solution multi-projets, avec des frontières de dépendances strictes. Elle prend en charge les types de connexion RDP, SSH, SFTP, FTP, VNC, Telnet, Citrix et Local Shell, avec 6 148 clés i18n par langue (EN/FR), 58 outils sysops intégrés dotés d'une aide contextuelle, une navigation croisée entre outils, et 10 801 tests automatisés (dont 10 670 bloquants en CI, tous verts, les 131 restants dans les lignes informatives CIUnstable et RequiresDesktop). Le moniteur de santé interroge en parallèle (Task.WhenAll), les importateurs XML sont durcis contre XXE, et tous les Debug.WriteLine ont été remplacés par FileLogger. Design System conforme WCAG AA avec 45 design tokens (typographie de 11 px minimum, espacements, rayons d'angle, opacités, tailles d'icônes, famille de police), micro-animations, FocusIndicatorBrush pour l'accessibilité clavier, système d'icônes unifié à deux niveaux (géométries vectorielles + MDL2), code couleur par catégorie d'outil, i18n déclarative via l'extension de balisage `{loc:Translate}` et ServerDialog à divulgation progressive.
...
Les changements du magasin de confiance atteignent `settings.json` par `HostKeyPersistenceCoalescer` (`Heimdall.App/Services/`) : chaque `EntryTrusted` / `EntryReplaced` / `EntryRemoved` et chaque `HostKeyStore.HostKeyEvent` est mis en file, et un seul `MergeSettingAsync` écrit le lot après une fenêtre calme de 250 ms, le dernier changement par clé l'emportant ; une vidange est lancée à la sortie avant le premier await et attendue à la fin. `HostKeyTrustService` ne balaie les entrées hachées que pour un hôte candidat en clair (un jeton haché ne peut apparier que lui-même) et énumère le magasin sans le copier. Côté session, `SshShellSession` atteint le réseau par un record interne `Transport` (résolution de la clé d'hôte, création du client, handshake, flux shell), ce qui permet aux tests de piloter sa boucle de lecture avec un flux scripté ; chaque await de `ConnectAsync` est suivi d'une vérification de disposition qui libère ce qu'un `Dispose` concurrent n'a pas pu voir. Le port de tunnel d'un pane n'est remis qu'une seule fois par `ConnectionStateMachine.TryTakeTunnelLocalPort`, si bien que le thread de sortie du processus et la fermeture depuis l'interface ne peuvent pas libérer deux fois la même acquisition, et `TunnelManager.AcquireReusableTunnel` honore la préférence de loopback distinct des lancements externes.
