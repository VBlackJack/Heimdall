...
-->
# Heimdall - Feature Reference

*Also available in French: [fr/FEATURES.md](fr/FEATURES.md).*

The complete catalogue of what Heimdall does, protocol by protocol. If you are looking for how to get started instead, read the [User Guide](USER-GUIDE.md); for the short version, the [README](../README.md).
...
- **Unified RDP import**: `.rdp` files dragged onto the main window or imported from `Settings → Import` go through the same preview/conflict resolution flow; replacing an existing profile keeps every field the file cannot carry (fixed resolution, saved password, post-connect steps); the import reads the keys the Remote Desktop client itself writes when it saves a file (`redirectdrives`, `domain`, COM ports, microphone, console session, compression, bitmap cache, auto-reconnect, USB, camera, dynamic resolution)
...
- **Folder colours**: the folder menu offers the eight palette colours project badges use, plus a way back to the default; the colour paints the folder icon, is inherited by sub-folders until they choose their own, follows a rename and goes with a deletion
- **Move folders by drag and drop**, or from the folder's "Move to" submenu: dropped on another folder it becomes its sub-folder, dropped on the drop zone above the tree it returns to the top level; sub-folders, sessions, colour and defaults travel with it, and a drop onto the folder itself, a sub-folder or the current parent is refused
- **Manual session order**: a session dropped on another session row lands before or after it (an accent edge shows the side), a drop on a row of another folder moves and positions in one write, Alt+Up and Alt+Down move the focused session one step; a sort order of zero means "no manual order" and ranks after every ordered session
...
- Trilingual interface: English, French and Spanish (6,439 i18n keys per language, exact parity across the three catalogues)
...
- External credential provider: choose a command-line provider (preset templates for KeePassXC, KeePass2/KPScript, Bitwarden CLI, 1Password CLI, pass - with an optional unlock secret fed via stdin, a separate username command, a per-profile vault entry name, and a "first line only" output mode) or the native Windows Credential Manager; the provider is built through `ICredentialProviderFactory` and covers SSH (embedded sessions and the Plink fallback), SFTP, RDP, WinRM in Credential mode, FTP and VNC. Telnet, Citrix and the external PuTTY mode do not consume credentials from this provider - database path browser, placeholder hints, test button with inline feedback; KeePassXC also supports key-file authentication (.keyx/.key), alone or combined with a master password
...
See [docs/ARCHITECTURE.md](ARCHITECTURE.md) for detailed design decisions and data flow diagrams.
