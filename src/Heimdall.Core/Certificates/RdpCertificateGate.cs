...
public static class RdpCertificateGate
{
    ...
    /// <remarks>
    /// <b>Only an explicit refusal stops the connection.</b> Every other outcome proceeds,
    /// including the two that mean "verified nothing" - an unreachable probe and an
    /// endpoint that offers no certificate. That is the same rule the verifier itself
    /// holds, seen from the other side: Heimdall may relax the Windows check only where it
    /// performed an equivalent one, and by symmetry it may not *tighten* the connection on
    /// the strength of a check that did not happen either. A probe failure that blocked
    /// the session would turn a verification step into a new way to fail, on a code path
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            return Decide(await verifyAsync(cancellationToken));
        }
        catch (Exception ex)
        {
